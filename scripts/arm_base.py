...
class Arm_base(object):
    def __init__(self,
        ...
        config_file_path = os.path.join(os.path.dirname(__file__),'config','Arm_2.yaml')
        try:
            with open(config_file_path, 'r') as file:
                self._config_data = yaml.safe_load(file)
                rospy.loginfo("data has been read")
        except Exception as e:
            rospy.logerr("path is not exits")
            return
        # self._ID = config_data["ID"]  # 每个机械臂的唯一标识
        self._color_information = None  # 服务器传来的颜色信息
        self._depth = int(self._config_data["depth"]) # 镜头距离物块的深度
        self._Arm_Location = (0, 0, 0)  # 坐标系的原点在机械臂的位置
        self._wucha = -1
        self._offset_change = float(self._config_data["offset_change"])  # 旋转角度误差 5
        self._offset_real_right = int(self._config_data["offset_real_right"]) # 误差界限_右 13
        self._offset_real_left = int(self._config_data["offset_real_left"]) # 误差界限_左 20

        self._wait_time = int(self._config_data["wait_time"])  # 等待索尼相机打开后稳定下来的时间
        self._error_range = int(self._config_data["error_range"])  # 识别框的误差允许范围
        self._error_range_width = int(self._config_data["error_range_width"])  # 识别框的宽度误差允许范围
        self._p_mould = [90, 110, 0, 0, 90]  # 机械臂的等待位置
    ...
    def recognize_angle(self, analyzer):
        ...
        print("the color information is:",self.get_color_information())
        # 启动串口
        ...
        while self._flag == False:
            print("ser.in_waiting is :", self._ser.in_waiting)
            if (self._ser.in_waiting > 0):
                result, self._flag = tool_box.setial_communicatiuon_mutiple(self._ser)
                
                if result is not None:
                    self._flag, self._coordinates = tool_box.check_coloridx_imformation(result, self.get_color_information())
                    ...
                    if self._flag_error == False:
                        ...
                        valid_data, self._flag_error = analyzer.analyze_new(self._coordinates[0], self._coordinates[2])
                        if self._flag_error:
    ...
    def arm_clamp_block(self, enable):
        if enable == 0:
            ...
            self._Arm.Arm_serial_servo_write(6, 135, 400)
    ...
    def control_Arm(self, id_, angle, flag, wucha):
        ...
        if flag == False:
            angle = self._Arm.Arm_serial_servo_read(id_) - (abs(angle) + abs(wucha))
            # time.sleep(1)
            rospy.sleep(1)
    ...
    def arm_centrality(self):
        ...
        self.arm_move(self._p_mould, 1000)
        # time.sleep(1)
        rospy.sleep(1)
