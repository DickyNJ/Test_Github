...
def setial_communicatiuon_mutiple(ser):
    ...
    if data[:4] == 'Data':
        ...
        if all(element == -1 for element in coordinates):
            rospy.logwarn("No estimated color information")
            return None, False
        else:
            return coordinates, True
...
def azimuthangle(point1, point2):
    rospy.loginfo("Type pt1: {}, type pt2: {}".format(type(point1), type(point2)))
    rospy.loginfo("Point 1: {}, Point 2: {}".format(point1, point2))
